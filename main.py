...
def MostrarGrafo():
    ...
    G = nx.Graph()
# Agregar nodos
    G.add_node(df.iloc[0,0].split(';')[0], color=diccionario[df.iloc[0,0].split(';')[2]])
    G.add_node(df.iloc[1,0].split(';')[0], color=diccionario[df.iloc[1,0].split(';')[2]])
    G.add_node(df.iloc[2,0].split(';')[0], color=diccionario[df.iloc[2,0].split(';')[2]])
# Verificar los nodos en el grafo
    print(G.nodes())
    nx.draw(G, with_labels=True, node_color=[G.nodes[n]['color'] for n in G.nodes])
    plt.show()
...
def Mostrarligas():
    ...
    imagen = Image.open("assets/LigaEspañola")
    ...
    imagen3 = Image.open("assets/Bundesliga")
